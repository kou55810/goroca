...
public class NetworkManager: MonoBehaviourPunCallbacks
{
    ...
    // 1. 部屋を作成して入室する
    public bool CreateAndJoinRoom(string roomName, int players)
    {
        // ルームオプションの基本設定
    ...
    public void JoinOrCreateRoom(string roomName, int players)
    {
        // ルームオプションの基本設定
        RoomOptions roomOptions = new RoomOptions
        {
    ...
    // 3. 特定の部屋に入室する
    public bool JoinRoom(string targetRoomName)
    {
        if (PhotonNetwork.InLobby)
        {
            return PhotonNetwork.JoinRoom(targetRoomName);
        }
        return false;
    }
